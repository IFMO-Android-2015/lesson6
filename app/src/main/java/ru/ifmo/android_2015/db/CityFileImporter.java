...
public abstract class CityFileImporter implements CityParserCallback {
    ...
    private SQLiteStatement insStatement = null;
    ...
    private void importCities(InputStream in) {
        ...
        db.beginTransaction();
        try {
            insStatement = db.compileStatement(
                    "INSERT INTO cities ("
    ...
    private boolean insertCity(SQLiteDatabase db,
                               ...
                               double latitude,
                               double longitude) {
        insStatement.bindLong(1, id);
        insStatement.bindString(2, name);
        insStatement.bindString(3, country);
        insStatement.bindDouble(4, latitude);
        insStatement.bindDouble(5, longitude);

        long rowId = insStatement.executeInsert();
        if (rowId < 0) {
