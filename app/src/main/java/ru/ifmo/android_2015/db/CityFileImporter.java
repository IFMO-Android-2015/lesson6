...
public abstract class CityFileImporter implements CityParserCallback {
    ...
    private SQLiteStatement ins;

    public CityFileImporter(SQLiteDatabase db) {
        this.db = db;
        ins = this.db.compileStatement(String.format("INSERT INTO %s " + "(%s, %s, %s, %s, %s) VALUES (?,?,?,?,?)",
                CityContract.Cities.TABLE,CityContract.CityColumns.CITY_ID,
    ...
    private boolean insertCity(SQLiteDatabase db,
        ...
        ins.bindString(3, country);
        ins.bindDouble(4, latitude);
        ins.bindDouble(5, longitude);
        long rowId = ins.executeInsert();
        if (rowId < 0) {
