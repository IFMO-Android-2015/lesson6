...
public abstract class CityFileImporter implements CityParserCallback {
    ...
    private SQLiteStatement compiledInsert = null;
    ...
    public final synchronized void importCities(File srcFile,
        ...
        db.beginTransaction();
        try {
            compiledInsert = db.compileStatement(
            ...
            db.endTransaction();
            if (compiledInsert != null) {
                try {
                    compiledInsert.close();
                    compiledInsert = null;
                } catch (Exception e) {
                    Log.e(LOG_TAG, "failed to close compiled statement");
                }
            }
            if (in != null) {
                try {
    ...
    private boolean insertCity(SQLiteDatabase db,
        ...
        compiledInsert.bindLong(1, id);
        compiledInsert.bindString(2, name);
        compiledInsert.bindString(3, country);
        compiledInsert.bindDouble(4, latitude);
        compiledInsert.bindDouble(5, longitude);

        long rowId = compiledInsert.executeInsert();
