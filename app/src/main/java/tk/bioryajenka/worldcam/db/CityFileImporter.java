...
import tk.bioryajenka.worldcam.json.CityParserCallback;
import tk.bioryajenka.worldcam.util.ObservableInputStream;
...
public abstract class CityFileImporter implements CityParserCallback {
    ...
    public final synchronized void importCities(File srcFile, ProgressCallback
    ...
    private void importCities(InputStream in) {
        ...
        statement = db.compileStatement(String.format("INSERT INTO %s " + "(%s, %s, %s, " +
                        "%s, %s) VALUES (?, ?, ?, ?, ?)", CityContract.Cities.TABLE,
                CityContract.Cities.CITY_ID, CityContract.Cities.NAME, CityContract.Cities
                        .COUNTRY, CityContract.Cities.LATITUDE, CityContract.Cities
                        .LONGITUDE));
        db.beginTransaction();
        try {
    ...
    private boolean insertCity(SQLiteDatabase db, long id, @NonNull String name, @NonNull
